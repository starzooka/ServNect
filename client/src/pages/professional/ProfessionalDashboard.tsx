...
export default function ProfessionalDashboard() {
  ...
  useEffect(() => {
    ...
    const initializeDashboard = async () => {
      ...
      // --- NEW: INITIALIZE ONLINE STATUS FROM DB ---
      ...
      // FETCH INITIAL LIVE JOBS
      fetchJobs(user.id);

      // SET UP SUPABASE REALTIME LISTENER
      realtimeSubscription = supabase
        .channel(`bookings_pro_${Date.now()}`) // <-- FIX: Made the channel name completely unique
        .on('postgres_changes', { 
          event: '*', 
          schema: 'public', 
          table: 'bookings',
  ...
  // --- NEW: TOGGLE ONLINE STATUS IN DATABASE ---
  const toggleOnlineStatus = async () => {
    ...
    if (error) {
      ...
      setIsOnline(!newStatus); // Revert if it fails
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-slate-900 border-slate-800 shadow-sm rounded-2xl">
            <CardContent className="p-5 flex flex-col justify-center">
              ...
              <p className="text-2xl font-bold text-white">0 Jobs</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-5 flex items-center justify-between h-full relative z-10">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <p className="text-amber-100 text-sm">No reviews yet</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <div className="flex gap-2 p-1 bg-slate-900 rounded-xl w-fit border border-slate-800">
            <button onClick={() => setActiveTab('requests')} className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'requests' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
              New Requests 
              {incomingRequests.length > 0 && <Badge variant="secondary" className={`${activeTab === 'requests' ? 'bg-amber-500/20 text-amber-500 border-none' : 'bg-slate-800 text-slate-500 border-none'}`}>{incomingRequests.length}</Badge>}
            </button>
            <button onClick={() => setActiveTab('schedule')} className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'schedule' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
              Upcoming Schedule
              {scheduledJobs.length > 0 && <Badge variant="secondary" className={`${activeTab === 'schedule' ? 'bg-amber-500/20 text-amber-500 border-none' : 'bg-slate-800 text-slate-500 border-none'}`}>{scheduledJobs.length}</Badge>}
            </button>
          </div>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between gap-4">
                        <div className="flex items-start gap-4">
                          <Avatar className="h-12 w-12 border border-slate-700">
                            <AvatarFallback className="bg-amber-500/10 text-amber-500">{req.customer_name[0]}</AvatarFallback>
                          </Avatar>
                          <div>
                        ...
                        <div className="flex flex-col sm:items-end gap-3 sm:gap-0 justify-between">
                          ...
                          <div className="flex gap-2 w-full sm:w-auto mt-2">
                            <Button variant="outline" onClick={() => updateJobStatus(req.id, 'declined')} className="flex-1 sm:flex-none border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 h-10 px-4 bg-transparent">Decline</Button>
                            <Button onClick={() => updateJobStatus(req.id, 'accepted')} className="flex-1 sm:flex-none bg-amber-500 hover:bg-amber-600 text-slate-950 font-semibold h-10 px-6 shadow-sm">Accept & Chat</Button>
                          </div>
                        </div>
                      </div>
                    </CardContent>
                      ...
                      <div className="flex flex-col sm:flex-row justify-between gap-4 sm:items-center">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <Badge variant="secondary" className="bg-green-500/10 text-green-500 border border-green-500/20 font-bold">Upcoming</Badge>
                        ...
                        <div className="flex items-center gap-4 bg-slate-950 p-3 rounded-xl border border-slate-800">
                          <Avatar className="h-10 w-10 border border-slate-700 shadow-sm">
                            <AvatarFallback className="bg-amber-500/10 text-amber-500">{job.customer_name[0]}</AvatarFallback>
                          </Avatar>
                          <div>
