...
import { Wrench, Settings, LogOut, ShieldAlert, MapPin, Clock, CheckCircle2, TrendingUp, Star, CalendarDays, BellRing } from "lucide-react";

const INCOMING_REQUESTS = [
  { id: 101, customer: "Vikram Singh", service: "AC Not Cooling", distance: "2.5 km", time: "Today, 4:00 PM", estRate: "₹500", img: "https://api.dicebear.com/7.x/avataaars/svg?seed=Vikram" },
  { id: 102, customer: "Neha Gupta", service: "Water Heater Repair", distance: "4.1 km", time: "Tomorrow, 10:00 AM", estRate: "₹650", img: "https://api.dicebear.com/7.x/avataaars/svg?seed=Neha" },
];

const SCHEDULED_JOBS = [
  { id: 201, customer: "Aarav Patel", service: "Washing Machine Installation", address: "B-402, Green Valley Apts", time: "Today, 1:00 PM", status: "Upcoming", img: "https://api.dicebear.com/7.x/avataaars/svg?seed=Aarav" },
];

export default function ProfessionalDashboard() {
  const navigate = useNavigate();
  const [userData, setUserData] = useState<{name: string, email: string, verified: boolean} | null>(null);
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (!profile || !profile.category || profile.category.trim() === '') {
        navigate('/onboarding', { replace: true });
        return;
      }

      setUserData({ name: user.user_metadata?.full_name || 'Professional', email: user.email || '', verified: profile.verified || false });
  ...
  const handleRestoreAccount = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (user) await supabase.from('professionals').update({ deletion_scheduled_at: null }).eq('id', user.id);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-11 pl-1.5 pr-4 rounded-full border border-slate-800 hover:bg-slate-900 gap-2.5 transition-all outline-none text-slate-300 hover:text-white">
                <Avatar className="h-8 w-8 border border-slate-700">
                  <AvatarFallback className="bg-amber-500/10 text-amber-500 font-bold text-xs">{userData?.name ? userData.name[0].toUpperCase() : 'P'}</AvatarFallback>
                </Avatar>
            ...
            <DropdownMenuContent className="w-56 p-2 rounded-2xl border-slate-800 shadow-xl bg-slate-900/95 backdrop-blur-xl mt-1 text-slate-300" align="end" forceMount>
              <DropdownMenuItem onClick={() => navigate('/settings')} className="group cursor-pointer font-medium p-3 rounded-xl focus:bg-slate-800 focus:text-amber-500 transition-colors">
                <Settings className="mr-3 h-4 w-4 text-slate-500 group-focus:text-amber-500 transition-colors" /> Account Settings
              </DropdownMenuItem>
              <div className="h-px bg-slate-800 my-1 mx-2"></div>
              <DropdownMenuItem onClick={() => setShowLogoutModal(true)} className="group cursor-pointer font-medium p-3 rounded-xl focus:bg-red-500/10 focus:text-red-500 transition-colors">
                <LogOut className="mr-3 h-4 w-4 text-red-500 transition-colors" /> Log out
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm">
          <div>
            ...
            <p className="text-slate-400 mt-1 flex items-center gap-2">
              {userData?.verified ? <><CheckCircle2 className="w-4 h-4 text-green-500" /> Verified Professional</> : <><Clock className="w-4 h-4 text-amber-500" /> Profile pending verification</>}
            </p>
          </div>
          <div className="flex items-center gap-4 bg-slate-950 p-2 pl-4 rounded-2xl border border-slate-800">
            <div className="flex flex-col">
              ...
              <span className="text-xs text-slate-500 font-medium">{isOnline ? 'Accepting jobs' : 'Not visible'}</span>
            </div>
            <button onClick={() => setIsOnline(!isOnline)} className={`relative inline-flex h-8 w-14 shrink-0 cursor-pointer items-center justify-center rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${isOnline ? 'bg-green-500' : 'bg-slate-700'}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="bg-slate-900 border-slate-800 shadow-sm rounded-2xl">
            <CardContent className="p-5 flex flex-col justify-center">
              <p className="text-sm font-medium text-slate-400 mb-1 flex items-center gap-1.5"><TrendingUp className="w-4 h-4" /> This Week</p>
          ...
          <Card className="bg-slate-900 border-slate-800 shadow-sm rounded-2xl">
            <CardContent className="p-5 flex flex-col justify-center">
              ...
              <p className="text-2xl font-bold text-white">12 Jobs</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-5 flex items-center justify-between h-full relative z-10">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  ...
                  <p className="text-amber-100 text-sm">/ 5.0 (42 reviews)</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <div className="flex gap-2 p-1 bg-slate-900 rounded-xl w-fit border border-slate-800">
            <button onClick={() => setActiveTab('requests')} className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'requests' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
              New Requests <Badge variant="secondary" className={`${activeTab === 'requests' ? 'bg-amber-500/20 text-amber-500 border-none' : 'bg-slate-800 text-slate-500 border-none'}`}>2</Badge>
            </button>
            <button onClick={() => setActiveTab('schedule')} className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'schedule' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
              Upcoming Schedule
            </button>
          </div>
            ...
            <div className="grid gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {!isOnline && (
                <div className="p-4 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-2xl flex items-start gap-3">
                  ...
                  <p className="text-sm font-medium">You are currently offline. Toggle your status to Online above to receive live requests from customers in your area.</p>
                </div>
              )}
                  ...
                  <CardContent className="p-5 sm:p-6">
                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <Avatar className="h-12 w-12 border border-slate-700"><AvatarImage src={req.img} /><AvatarFallback>{req.customer[0]}</AvatarFallback></Avatar>
                        <div>
                          <h3 className="font-bold text-lg text-white leading-tight">{req.service}</h3>
                          <p className="text-slate-400 text-sm font-medium mt-0.5">{req.customer}</p>
                          <div className="flex items-center gap-4 mt-3">
                            <span className="text-xs font-semibold text-slate-300 flex items-center gap-1 bg-slate-950 border border-slate-800 px-2.5 py-1 rounded-md"><MapPin className="w-3.5 h-3.5"/> {req.distance}</span>
                            <span className="text-xs font-semibold text-slate-300 flex items-center gap-1 bg-slate-950 border border-slate-800 px-2.5 py-1 rounded-md"><Clock className="w-3.5 h-3.5"/> {req.time}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col sm:items-end gap-3 sm:gap-0 justify-between">
                        <div className="text-left sm:text-right">
                          ...
                          <p className="text-xl font-bold text-amber-500">{req.estRate}</p>
                        </div>
                        <div className="flex gap-2 w-full sm:w-auto mt-2">
                          <Button variant="outline" className="flex-1 sm:flex-none border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 h-10 px-4 bg-transparent">Decline</Button>
                          <Button className="flex-1 sm:flex-none bg-amber-500 hover:bg-amber-600 text-slate-950 font-semibold h-10 px-6 shadow-sm">Accept & Chat</Button>
                        </div>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between gap-4 sm:items-center">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="secondary" className="bg-green-500/10 text-green-500 border border-green-500/20 font-bold">{job.status}</Badge>
                          <span className="text-sm font-semibold text-slate-400 flex items-center gap-1"><CalendarDays className="w-4 h-4"/> {job.time}</span>
                        </div>
                        <h3 className="font-bold text-lg text-white mt-2">{job.service}</h3>
                        <p className="text-slate-400 text-sm mt-1 flex items-center gap-1"><MapPin className="w-4 h-4"/> {job.address}</p>
                      </div>
                      <div className="flex items-center gap-4 bg-slate-950 p-3 rounded-xl border border-slate-800">
                        <Avatar className="h-10 w-10 border border-slate-700 shadow-sm"><AvatarImage src={job.img} /><AvatarFallback>{job.customer[0]}</AvatarFallback></Avatar>
                        <div>
                          <p className="text-xs text-slate-500 font-medium">Customer</p>
                          <p className="text-sm font-bold text-white">{job.customer}</p>
                        </div>
                        <Button variant="outline" size="sm" className="ml-2 bg-transparent border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white h-8">Message</Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
