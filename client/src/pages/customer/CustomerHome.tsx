...
export default function CustomerHome() {
  ...
  // FIX 1: Store userId in a ref so it's accessible inside async callbacks
  // without needing to add it as a useEffect dependency.
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      if (profileData?.deletion_scheduled_at) {
        ...
        setDeletionDates({
          scheduledAt: startDateOriginal.toLocaleDateString(),
          deletedOn: startDate.toLocaleDateString(),
        });
  ...
  useEffect(() => {
    ...
    if (activeChat) {
      // FIX 2: Clear old messages when opening a new chat so stale
      // messages from a previously opened chat don't flash on screen.
      setMessages([]);
      fetchMessages(activeChat.id);

      chatSubscription = supabase
        .channel(`chat_customer_${activeChat.id}`)
        .on(
          'postgres_changes',
          { event: 'INSERT', schema: 'public', table: 'booking_messages', filter: `booking_id=eq.${activeChat.id}` },
          (payload: any) => {
  ...
  const fetchMyBookings = async (customerId: string) => {
    const { data, error } = await supabase
      .from('bookings')
      .select(`*, professional:professionals(full_name, category)`)
  ...
  // FIX 3: Added the missing fetchMessages function that was called inside
  // the chat useEffect but never defined, causing a runtime ReferenceError.
  ...
  const submitReview = async () => {
    ...
    if (!error) {
      ...
      // FIX 4: Also reset the review form fields after a successful submission.
      setReviewRating(5);
      setReviewComment('');
      fetchProfessionals();
    }
        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-8 text-center space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-slate-50 text-slate-700 p-4 rounded-xl text-sm leading-relaxed border border-slate-200 mt-4 text-left">
              Your account is currently under deletion since <strong>{deletionDates.scheduledAt}</strong> and
              will be permanently deleted on <strong>{deletionDates.deletedOn}</strong>.
            </div>
          </div>
          <div className="space-y-3 pt-4">
            ...
            <Button
              ...
              variant="outline"
              className="w-full border-slate-300 text-slate-700 font-semibold h-12 hover:bg-slate-50"
            >
              Log Out
            </Button>
          </div>
      ...
      {cancelBookingId && (
        <div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 text-center">
            ...
            <div className="flex flex-col gap-3">
              <Button onClick={executeCancelRequest} className="bg-red-600 hover:bg-red-700 text-white">
                Yes, Cancel Request
              </Button>
              <Button onClick={() => setCancelBookingId(null)} variant="outline">
                No, Keep It
              </Button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 text-center relative">
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  ...
                  className={`w-10 h-10 cursor-pointer ${
                    reviewRating >= star ? 'text-amber-500 fill-amber-500' : 'text-slate-200 fill-slate-200'
                  }`}
                />
              ))}
            </div>
            <div className="mb-6 text-left">
              ...
              <textarea
                ...
                className="w-full mt-2 min-h-[80px] p-3 rounded-xl border border-slate-200 bg-slate-50"
              />
            </div>
            <Button
              ...
              <div className="flex items-center gap-3">
                <Avatar className="h-10 w-10">
                  <AvatarFallback className="bg-blue-100 text-blue-600">
                    {activeChat.professional?.full_name?.[0] || 'P'}
                  </AvatarFallback>
                </Avatar>
                <div>
                  {/* FIX 5: Guard against undefined full_name with optional chaining */}
                  <h3 className="font-bold">{activeChat.professional?.full_name ?? 'Professional'}</h3>
                </div>
              </div>
              <button onClick={() => setActiveChat(null)} className="p-2 hover:bg-slate-100 rounded-full">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
              <div className="text-center pb-4">
                <Badge variant="outline">Job accepted. You can now chat!</Badge>
              </div>
            ...
            <div className="p-4 border-t bg-white shrink-0">
              <form onSubmit={sendMessage} className="flex gap-2 relative">
                ...
                <Button
                  ...
                  size="icon"
                  className="absolute right-1 top-1 h-10 w-10 bg-blue-600"
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex bg-slate-100/80 p-1 rounded-full border border-slate-200 mx-4">
            <button
              ...
              className={`px-5 py-2 text-sm font-bold rounded-full ${
                activeTab === 'discover' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'
              }`}
            >
              Discover
            </button>
            <button
              ...
              className={`px-5 py-2 text-sm font-bold rounded-full flex items-center gap-2 ${
                activeTab === 'bookings' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'
              }`}
            >
              My Bookings
              {myBookings.some(b => b.status === 'accepted') && (
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-11 pl-1.5 pr-4 rounded-full border">
