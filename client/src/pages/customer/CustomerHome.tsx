...
import { 
  MapPin, Search, Star, Wrench, Settings, LogOut, ShieldCheck, Zap, Droplets, Hammer, Paintbrush, ShieldAlert, X, Clock, IndianRupee, CalendarDays, CheckCircle2, MessageSquare, Send
} from "lucide-react";
...
export default function CustomerHome() {
  ...
  useEffect(() => {
    ...
    if (activeChat) {
      ...
      chatSubscription = supabase.channel(`chat_customer_${activeChat.id}_${Date.now()}`)
  ...
  const handleBookService = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('bookings').insert([{
      customer_id: userData.id, professional_id: selectedPro.id, customer_name: userData.name,
      service_type: bookingForm.issue, scheduled_time: 'To be discussed', address: bookingForm.address,
      est_budget: 'To be discussed', status: 'pending'
    }]);
    setIsBooking(false);
  ...
  const executeCancelRequest = async () => {
    ...
    if (error) {
      ...
      if (userData?.id) fetchMyBookings(userData.id);
    }
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('booking_messages').insert([{
      ...
      professional_id: activeChat.professional_id,
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-8 text-center space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="space-y-3 pt-4">
            ...
            <Button onClick={handleLogout} variant="outline" className="w-full border-slate-300 text-slate-700 font-semibold h-12 hover:bg-slate-50">Go Back</Button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 relative">
      ...
      {cancelBookingId && (
        <div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-sm w-full p-8 shadow-2xl animate-in zoom-in-95 duration-300 relative text-center">
            <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4"><X className="w-8 h-8 text-red-600" /></div>
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Cancel Request?</h2>
            <p className="text-slate-500 text-sm mb-6 leading-relaxed">Are you sure you want to cancel this booking request? The professional will no longer be able to see or accept it.</p>
            <div className="flex flex-col gap-3">
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-300 relative">
            ...
            <div className="flex items-center gap-4 mb-6 border-b border-slate-100 pb-6">
              <Avatar className="h-14 w-14 border border-slate-100 shadow-sm"><AvatarFallback className="bg-blue-100 text-blue-700 font-bold">{selectedPro.full_name[0]}</AvatarFallback></Avatar>
              <div><h2 className="text-xl font-bold text-slate-900">Book {selectedPro.full_name.split(' ')[0]}</h2><p className="text-sm text-slate-500">{selectedPro.category}</p></div>
            </div>
            <form onSubmit={handleBookService} className="space-y-4">
              <div className="space-y-2"><Label>What is the issue?</Label><div className="relative flex items-center"><Wrench className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input required value={bookingForm.issue} onChange={(e) => setBookingForm({...bookingForm, issue: e.target.value})} placeholder="e.g. Leaking kitchen sink" className="pl-9 h-11" /></div></div>
              <div className="space-y-2"><Label>Your Address</Label><div className="relative flex items-center"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input required value={bookingForm.address} onChange={(e) => setBookingForm({...bookingForm, address: e.target.value})} placeholder="e.g. Apt 4B, Green Valley" className="pl-9 h-11" /></div></div>
              ...
              <div className="bg-blue-50 p-3.5 rounded-xl border border-blue-100 flex items-start gap-3 mt-4">
                 ...
                 <p className="text-xs text-blue-800 leading-relaxed">
                   <strong>Next Step:</strong> Once the professional accepts your request, you can message them to discuss the exact date, time, and budget directly in the chat!
                 </p>
              </div>

              <Button type="submit" disabled={isBooking} className="w-full h-12 text-base font-bold bg-blue-600 hover:bg-blue-700 text-white mt-2">{isBooking ? "Sending Request..." : "Confirm Booking Request"}</Button>
            </form>
          </div>
        </div>
      )}
          ...
          <div className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full h-[85vh] shadow-2xl animate-in zoom-in-95 duration-300 relative flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white z-10 shrink-0">
              <div className="flex items-center gap-3">
                <Avatar className="h-10 w-10 border border-slate-200"><AvatarFallback className="bg-blue-100 text-blue-600">{activeChat.professional?.full_name[0] || 'P'}</AvatarFallback></Avatar>
                <div><h3 className="font-bold text-slate-900 leading-tight">{activeChat.professional?.full_name || 'Professional'}</h3><p className="text-xs text-slate-500">{activeChat.professional?.category || 'Service Partner'}</p></div>
              </div>
              <button onClick={() => setActiveChat(null)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
              <div className="text-center pb-4"><Badge variant="outline" className="text-slate-500 bg-white">Job accepted. You can now chat!</Badge></div>
              {messages.map((msg, idx) => {
                const isMe = msg.sender_id === userData?.id;
                return (
                  <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${isMe ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-white text-slate-800 rounded-bl-sm border border-slate-200 shadow-sm'}`}>
                      {msg.content}
                    </div>
                  </div>
                );
            ...
            <div className="p-4 border-t border-slate-100 bg-white shrink-0">
              <form onSubmit={sendMessage} className="flex gap-2 relative">
                <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message to finalize details..." className="h-12 bg-slate-50 border-slate-200 text-slate-900 focus-visible:ring-blue-600 rounded-xl pr-12" />
                <Button type="submit" disabled={!newMessage.trim()} size="icon" className="absolute right-1 top-1 h-10 w-10 rounded-lg bg-blue-600 hover:bg-blue-700 text-white shrink-0"><Send className="w-4 h-4" /></Button>
              </form>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 p-2 rounded-2xl border-slate-200 shadow-xl bg-white/95 backdrop-blur-xl mt-1" align="end" forceMount>
              ...
              <DropdownMenuItem onClick={() => navigate('/settings')} className="group cursor-pointer text-slate-700 font-medium p-3 rounded-xl focus:bg-slate-50 focus:text-blue-600 transition-colors"><Settings className="mr-3 h-4 w-4 text-slate-500 group-focus:text-blue-600 transition-colors" /> Account Settings</DropdownMenuItem>
              <div className="h-px bg-slate-100 my-1 mx-2"></div>
              <DropdownMenuItem onClick={() => setShowLogoutModal(true)} className="group cursor-pointer text-red-600 font-medium p-3 rounded-xl focus:bg-red-50 focus:text-red-700 transition-colors"><LogOut className="mr-3 h-4 w-4 text-red-600 group-focus:text-red-700 transition-colors" /> Log out</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 animate-in fade-in duration-500">
        {activeTab === 'discover' && (
          <>
            <div className="space-y-6">
              <div><h1 className="text-3xl font-bold tracking-tight">Good morning, {firstName}! 👋</h1><p className="text-slate-500 mt-1">What do you need help with today?</p></div>
              <div className="relative max-w-2xl"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" /><Input type="text" placeholder="Search for plumbers, electricians, cleaners..." className="pl-10 h-14 rounded-2xl text-lg shadow-sm border-slate-300 focus-visible:ring-blue-600 bg-white text-slate-900" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
            </div>
            {!locationEnabled && (
              <Card className="bg-blue-50 border-blue-100 shadow-none">
                <CardContent className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3 text-blue-900"><div className="p-2 bg-blue-100 rounded-full"><MapPin className="h-5 w-5 text-blue-600" /></div><div><p className="font-semibold">Find professionals near you</p><p className="text-sm text-blue-700/80">Enable location services for accurate results and faster bookings.</p></div></div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                <button onClick={() => setActiveCategory(null)} className={`flex items-center gap-2 px-5 py-3 rounded-2xl border transition-all shrink-0 font-medium ${!activeCategory ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'border-slate-200 bg-white hover:bg-slate-50 text-slate-700'}`}>All Services</button>
                {CATEGORIES.map((cat, i) => (
                  <button key={i} onClick={() => setActiveCategory(cat.name)} className={`flex items-center gap-2 px-5 py-3 rounded-2xl border transition-all shrink-0 font-medium ${activeCategory === cat.name ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'border-slate-200 bg-white hover:bg-slate-50 text-slate-700'}`}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between"><h2 className="text-xl font-bold">Recommended Professionals</h2>{locationEnabled && <span className="text-sm text-green-600 font-medium flex items-center gap-1"><MapPin className="h-3 w-3"/> Near you</span>}</div>
              {filteredPros.length === 0 ? (
                <div className="text-center p-12 border border-dashed border-slate-300 rounded-3xl bg-white text-slate-500"><p>No professionals found for your search criteria.</p></div>
              ) : (
        ...
        {activeTab === 'bookings' && (
          <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-right-4">
            <div><h1 className="text-3xl font-bold tracking-tight">Your Activity</h1><p className="text-slate-500 mt-1">Track the status of your service requests.</p></div>
            {myBookings.length === 0 ? (
              <div className="text-center p-12 border border-dashed border-slate-300 rounded-3xl bg-white text-slate-500"><p>You haven't made any booking requests yet.</p><Button onClick={() => setActiveTab('discover')} variant="outline" className="mt-4 border-slate-300 text-blue-600 font-bold hover:bg-slate-50">Browse Professionals</Button></div>
            ) : (
              <div className="space-y-4">
                    ...
                    <Card key={booking.id} className="border-slate-200 shadow-sm bg-white rounded-2xl overflow-hidden">
                      ...
                      <CardContent className="p-5 sm:p-6">
                        <div className="flex flex-col sm:flex-row justify-between gap-4">
                          <div>
                            <div className="mb-3">{statusBadge}</div><h3 className="font-bold text-lg text-slate-900 leading-tight">{booking.service_type}</h3>
                          ...
                          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col justify-center sm:min-w-[200px]">
                            ...
                            <div className="flex items-center gap-3"><Avatar className="h-10 w-10 border border-slate-200 shadow-sm"><AvatarFallback className="bg-blue-100 text-blue-700 font-bold">{booking.professional?.full_name[0] || 'P'}</AvatarFallback></Avatar><div><p className="font-bold text-slate-900 text-sm leading-tight">{booking.professional?.full_name || 'Loading...'}</p><p className="text-slate-500 text-xs">{booking.professional?.category || 'Professional'}</p></div></div>
