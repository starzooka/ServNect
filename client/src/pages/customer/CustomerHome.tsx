...
export default function CustomerHome() {
  ...
  // NEW: Store our dynamic categories
  ...
  const fetchProfessionals = async () => {
    ...
    if (!error && data) {
      ...
      // EXTRACT DYNAMIC CATEGORIES HERE
      const uniqueCats = Array.from(new Set(enrichedPros.map((p: any) => p.category))).filter(Boolean) as string[];
  ...
  const handleBookService = async (e: React.FormEvent) => {
    ...
    setIsBooking(false);
    if (error) alert("Failed to send booking request.");
    else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56 p-2 rounded-2xl" align="end">
              <DropdownMenuItem onClick={() => navigate('/settings')} className="cursor-pointer"><Settings className="mr-3 h-4 w-4" /> Account Settings</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setShowLogoutModal(true)} className="cursor-pointer text-red-600"><LogOut className="mr-3 h-4 w-4" /> Log out</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {activeTab === 'bookings' && <BookingsTab myBookings={myBookings} setActiveTab={setActiveTab} setActiveChat={setActiveChat} setCancelBookingId={setCancelBookingId} setReviewBooking={setReviewBooking} setReviewRating={setReviewRating} setReviewComment={setReviewComment} />}
      </main>
    </div>
  );
}