...
export default function CustomerHome() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (user) {
        ...
        // MOCK LOGIC: In a real app, query your public.customers table here 
        // to check if `deletion_scheduled_at` is NOT null.
        // Example:
        // const { data } = await supabase.from('customers').select('deletion_scheduled_at').eq('id', user.id).single();
        // if (data?.deletion_scheduled_at) setShowRestoreModal(true);
  ...
  const handleRestoreAccount = async () => {
    // MOCK LOGIC: Update your database here to set `deletion_scheduled_at = null`
    // Example:
    // const { data: { user } } = await supabase.auth.getUser();
    // await supabase.from('customers').update({ deletion_scheduled_at: null }).eq('id', user?.id);
  ...
  // --- INTERCEPTOR: ACCOUNT RESTORATION MODAL ---
  if (showRestoreModal) {
    return (
        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-8 text-center space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-500 text-sm leading-relaxed">
              Your account is currently deactivated and is scheduled to be permanently wiped. Would you like to cancel the deletion process and restore your account?
            </p>
          </div>
          <div className="space-y-3 pt-4">
            <Button onClick={handleRestoreAccount} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 text-lg font-semibold">
        ...
        {!locationEnabled && (
          <Card className="bg-blue-50 border-blue-100 shadow-none">
            <CardContent className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <Button onClick={requestLocation} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold shrink-0">
                Enable Location
              </Button>
            </CardContent>
          </Card>
              ...
              <Card key={pro.id} className="border-slate-200 hover:shadow-lg transition-shadow group cursor-pointer bg-white">
                <CardContent className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-lg leading-tight flex items-center gap-1 text-slate-900">
                          {pro.name} {pro.verified && <ShieldCheck className="h-4 w-4 text-blue-600" />}
                        </h3>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    {locationEnabled ? (
                      <span className="text-slate-500 flex items-center gap-1"><MapPin className="h-3 w-3"/> {pro.distance}</span>
                    ) : null}
                  </div>

                  <Button className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold transition-transform active:scale-95">
                    View Profile
                  </Button>
                </CardContent>
              </Card>
