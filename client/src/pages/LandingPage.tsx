...
import { 
  ...
  ThumbsUp, CircleDollarSign, BadgeCheck, MapPin, CheckCircle2, Menu
} from "lucide-react";
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-slate-50 overflow-x-hidden text-slate-900 selection:bg-blue-200 selection:text-blue-900">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-slate-200' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600">
            ...
            <a href="#advantage" className="hover:text-blue-600 transition-colors">Why ServNect</a>
            <a href="#pro" className="hover:text-amber-600 transition-colors">Become a Pro</a>
          </div>

          {/* Actions */}
          <div className="hidden md:flex items-center gap-4">
            <Button variant="ghost" className="text-slate-600 hover:text-blue-600" onClick={() => navigate('/login')}>
              Log In
            </Button>
            <Button className="bg-blue-600 hover:bg-blue-700 text-white rounded-full px-6" onClick={() => navigate('/signup')}>
              Sign Up
            </Button>
          </div>
      ...
      {/* --- 1. HERO SECTION --- */}
      <section className="relative flex flex-col items-center justify-center px-6 pt-40 pb-24 md:pt-48 md:pb-32 text-center min-h-[95vh] overflow-hidden">
        ...
        <FadeInOnScroll className="relative z-10 max-w-5xl space-y-8 w-full">
          ...
          <div className="max-w-2xl mx-auto mt-12 bg-white/70 backdrop-blur-xl p-2.5 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 flex items-center transition-all duration-500 hover:shadow-[0_8px_40px_rgb(37,99,235,0.12)] focus-within:ring-2 focus-within:ring-blue-500/30 focus-within:bg-white">
      ...
      {/* Kept the background consistent with slate-50, using a subtle border to separate sections */}
      <section id="services" className="py-32 px-6 border-t border-slate-200/50 relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <FadeInOnScroll key={index} delay={index * 100}>
                <div className="group p-8 rounded-3xl border border-slate-100 bg-white shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col items-start gap-5 relative overflow-hidden">
      ...
      <section id="advantage" className="py-32 px-6 border-t border-slate-200/50">
            ...
            <FadeInOnScroll className="flex-1 space-y-10 order-2 lg:order-1">
              <div className="space-y-4">
                <h2 className="text-4xl md:text-5xl font-bold leading-tight tracking-tight">
                  Why trust <span className="text-blue-600">ServNect</span>!
                </h2>
                <p className="text-xl text-slate-600 font-light">
                  We empower you to choose. Connect directly with professionals, compare transparent prices, and hire with confidence.
                </p>
              </div>
      ...
      {/* Kept dark to strongly differentiate the B2B/Partner aspect of the app */}
      <section id="pro" className="relative overflow-hidden group">
